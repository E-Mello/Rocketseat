...
export function TaskList() {
    ...
    const [qtdMarked, setQtdMarked] = useState<TaskItem[]>([]);
    ...
    function handleMarkdownTask(id:number) {
        ...
        if(isChecked == true) {
            const mark = tasks.filter(task => task.isComplete === true).length()
            }
            setQtdMarked([...qtdMarked, ]);
        } else {
            const receivedTasks = tasks.filter((task) =>{
                return !(task.id === id);
            })
            setQtdMarked(receivedTasks);
        }
    }

    //     onChange={() => { handleMarkdownTask(id) }} 
            ...
            <div className={styles.tasks}>
                <div className={styles.info}>
                    ...
                    <div className={styles.done}>
                        ...
                        <button className={styles.counter}>
                            <p className={styles.pResult}>{qtdMarked.length} de {tasks.length}</p>
                        </button>
                    </div>
                </div>
